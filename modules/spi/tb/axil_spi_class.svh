...
class axil_spi_class #(
    ...
    task automatic spi_read_regs();
        ...
        begin
            ...
            env.master_read_reg(BASE_ADDR + ADDR_OFFSET * SPI_CLK_DIVIDER_REG_POS, spi_regs.clk_divider);
            env.master_read_reg(BASE_ADDR + ADDR_OFFSET * SPI_WAIT_TIME_REG_POS, spi_regs.wait_time);
            ...
            $display("[%0t][SPI]: bytes_num     = %0d", $time, spi_regs.control.bytes_num);
    ...
    task automatic spi_start();
        ...
        spi_regs.control.bytes_num = BYTES_NUM;
        begin
            env.master_write_reg(BASE_ADDR + ADDR_OFFSET * SPI_WAIT_TIME_REG_POS, spi_regs.wait_time);
            env.master_write_reg(BASE_ADDR + ADDR_OFFSET * SPI_CLK_DIVIDER_REG_POS, spi_regs.clk_divider);
            env.master_write_reg(BASE_ADDR + ADDR_OFFSET * SPI_SLAVE_SELECT_REG_POS, spi_regs.slave);
